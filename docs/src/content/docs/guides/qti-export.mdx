...
correct, page path). Enable it in the plugin:

```js title="astro.config.mjs"
starlight({ plugins: [starlightQuiz({ manifest: true })] });
```

It is written to your build output (`dist/quiz-manifest.json`) by parsing the generated HTML, so you have to **build**
the site first (`npx astro build`); it is not produced by the dev server. See
[Configuration](/starlight-quiz/guides/configuration/#manifest) for the option, and the
[CLI guide](/starlight-quiz/guides/cli/#the-quiz-manifest) for more on what the manifest is.

## Export

Point `export-qti` at a local build directory (or the manifest file), and choose an output directory:
...
| Flag         | Description                                      |
| ------------ | ------------------------------------------------ |
| `<source>`   | A manifest file, a directory containing one, or a deployed site URL. |
