...
:::caution[Only `[x]`, `[ ]` and `[X]` are recognised]
A mistyped marker — `[o]`, an empty `[]`, or a “smart” bracket pasted from an editor — isn't a checkbox in markdown, so
it would silently never be an answer. The build fails with an error pointing at it (disable with the
