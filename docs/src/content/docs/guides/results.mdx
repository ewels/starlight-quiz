...
description: Add an aggregate score panel and confetti to a page of quizzes.
---
...
<Quiz title="Question one">…</Quiz>
<Quiz title="Question two">…</Quiz>
...
<Quiz title="Question one" id="results-q1">
...
<Quiz title="Question two" id="results-q2">
...
Set the `confetti` prop to celebrate a first-time completion (when the score is at least 10 %). The confetti library is
loaded with a dynamic import, so it adds nothing to your bundle unless it actually fires.

## Resetting

The results panel includes a **Reset all answers** button that clears every quiz on the page. Each quiz also keeps its
own reset button when `disableAfterSubmit={false}`.
