...
[`progressTracker`](/starlight-quiz/guides/configuration/#plugin-options) plugin option. To place it somewhere specific
(or to use it [without Starlight](/starlight-quiz/guides/vanilla-astro/)), import and drop in the
[`<QuizProgress />`](/starlight-quiz/guides/configuration/#quizprogress-props) component yourself.
...
| `starlight-quiz:reset-all`  | _(none)_ — fired when every quiz on the page is reset                        |
...
The `progress` event fires whenever progress changes — including as each quiz registers on load (restoring saved
