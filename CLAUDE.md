...
`lib/strings.ts` holds the English strings (the single source of truth and the vanilla-Astro fallback). `translations.ts` is **generated** — `scripts/build-i18n.ts` compiles the gettext `.po` files in `locales/` into per-locale tables, mapping each namespaced key to its msgid. **Every** string lives in the `.po` files — there is no second source. The shared strings are byte-identical to mkdocs-quiz's own `.po` files so the sibling plugins read the same way; the handful with no mkdocs counterpart (intro panel, ToC badge, page-wide reset and its confirm prompt) are appended in a marked block at the end of each file. That block is inert for mkdocs-quiz, which loads a `.po` into a dict and looks up only the msgids it knows. It ships 13 locales; each table overlays only the keys it translates, with Starlight/`resolveString` falling back to English. After editing a `.po` file, run `pnpm --filter starlight-quiz gen:i18n` and commit the regenerated `translations.ts`. The result is injected via Starlight's `i18n:setup`; components resolve each label through `resolveString` — translation table when under Starlight, explicit prop override otherwise.
